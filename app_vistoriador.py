# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# Painel de Produção por Vistoriador (Streamlit) - MULTI-MESES (TOKYO VISTORIAS)
...
# CONFIG BÁSICA
# =========================
st.set_page_config(page_title="🧰 Produção por Vistoriador - TOKYO VISTORIAS (multi-meses)", layout="wide")
st.title("🧰 Painel de Produção por Vistoriador - TOKYO VISTORIAS")

# === Planilha-Índice (ARQUIVOS) ===
# Agora o ID vem do secrets.toml (local do projeto):
# No arquivo .streamlit/secrets.toml, coloque:
# tokyo_index_sheet_id = "ID_DA_SUA_PLANILHA_INDICE_TOKYO"
INDEX_SHEET_ID = st.secrets.get("tokyo_index_sheet_id", "").strip()
INDEX_TAB_NAME = "ARQUIVOS"

if not INDEX_SHEET_ID:
    st.error("⚠️ Defina no secrets.toml a chave tokyo_index_sheet_id com o ID da planilha-índice da TOKYO VISTORIAS.")
    st.stop()
